...
- Install exactly one skin pack, and unless I ask for a stock pack by name, install `http://127.0.0.1:3000/r/skin-flat.json`. Core owns the mechanics and the skin owns every token value, so with no pack installed there is no fallback to render against.
...
- Both list exactly the items you installed. http://127.0.0.1:3000/r/update.json is the complete component set, so it belongs in the scripts only if you installed the complete set.
