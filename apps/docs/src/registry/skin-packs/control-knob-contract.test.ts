...
const KNOB_SUFFIX =
  /-(?:background(?:-image)?|foreground|border-(?:color|width|style)|(?:ring|line|dot|marker|separator|handle)-color|radius|shadow|fill|stroke|backdrop-(?:filter|blur)|opacity|scale|gap|icon|height|size|padding(?:-inline)?|font-(?:size|weight)|easing|animation-duration|transition-(?:duration|delay)|indicator-(?:start|middle|end))$/;
...
describe("knob naming", () => {
  ...
  test("ends in a vocabulary suffix", () => {
    expect(knobsOutsideSuffixVocabulary(Object.values(controlKnobContracts).flat())).toEqual([]);
  });
