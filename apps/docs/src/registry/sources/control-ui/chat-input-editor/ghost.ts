...
// Fix: clone element into a fixed-position ghost over its last rect, let editor update immediately, animate ghost out via data-chat-input-exit/data-exiting (chat-input-editor.css blur keyframe); self-removes on animationend + timeout backstop.
...
export function spawnExitGhost(source: HTMLElement, inherit: readonly string[] = []): void {
  ...
  ghost.setAttribute("data-chat-input-exit", "");
