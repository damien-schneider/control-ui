...
- Install the complete set with neutral defaults in one command: \`npx shadcn@latest add ${normalizedOrigin}/r/all.json\`. Picking components one by one leaves the skin we write next with almost no surface to be checked against, and forces the update scripts below to track a bespoke list.
- Core supplies a complete neutral baseline with no skin config or provider. The application owns its theme; use its CSS to customize tokens. Optional named presets are separate installs.
- Core supplies a neutral baseline with squared corners and hairline borders. App-owned token declarations override the zero-specificity defaults. Between this install and the theme below, the app looks rougher than before — say so when you report the install, and carry the theme through in this same run. The reset is scaffolding, never the state you leave me in.
...
- No data-skin stamp or provider is required for the default install. A named scoped skin uses matching CSS selectors and a SkinProvider around the app; pass its config from a client module so portals receive the same id.
- The core token names match shadcn. Application :root and .dark tokens override the zero-specificity library defaults; preserve and adapt them to the selected design. Check app @theme bindings for conflicts with Control UI bindings, and show any necessary cleanup as a focused diff.
- Run \`node <installed control-ui directory>/scripts/control-ui-doctor.mjs\` and resolve import and binding errors before styling. For optional named presets, it also checks the data-skin stamp.
