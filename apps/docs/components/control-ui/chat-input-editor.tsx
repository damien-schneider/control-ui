...
import { useChatInputContext } from "@/components/control-ui/chat-input";
import type { ChatInputSubmitPayload } from "@/components/control-ui/contracts";
import { cn } from "@/components/control-ui/lib/cn";
import { MESSAGE_GHOST_INHERIT, spawnExitGhost } from "./chat-input-editor/ghost";
import { createEditorSchema } from "./chat-input-editor/schema";
import { docFromText, serializeDoc } from "./chat-input-editor/serialize";
import type { ChatInputEditorApi, ChatInputEditorProps } from "./chat-input-editor/types";
...
// Doc is source of truth: pushes serialized text to chat-input hook; re-hydrates only on EXTERNAL value changes (clear/prefill), never own keystrokes (would fight caret).
export function ChatInputEditor({ className, placeholder, extensions = [] }: ChatInputEditorProps) {
  ...
  useEffect(() => {
    ...
    const submitMessage = (editorState: EditorState) => {
      const extra: Partial<ChatInputSubmitPayload> = {};
  ...
  return (
    <div data-control-ui="chat-input-editor" data-slot="root" className={cn("relative", className)}>
