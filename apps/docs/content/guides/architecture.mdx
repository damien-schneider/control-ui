...
<GuideSection id="skin-cost" title="Keep the active skin sparse">
  ...
  Bundle tests cap every shipped config at 4 kB gzip. Refined remains nearly empty; richer packs pay only for their
...
<GuideSection id="registry" title="Registry source of truth">
  ...
  The current anatomy contract is version 9. Version 6 removed runtime class and paint maps: component visuals now flow through
  ...
  Version 9 folds `code-block-editor` into `code`: one paint family, one knob set, with the editable textarea as the
  `editor` slot.
